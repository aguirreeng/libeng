...
.. module:: drag_laws
    :synopsis: Drag laws for particles, droplets and bubbles.
...
def SchillerNaumann1933(reynolds, modified=True):
    '''
    .. py:function:: SchillerNaumann1933(reynolds, [modified=True])
    
    Function to calculate drag coefficient of a spherical particle
    using the correlation by Schiller & Naumann (1933) with the 
    modification found at ANSYS Inc. (2013).
    
    :param float reynolds: Reynolds number based on fluid properties 
    and relative velocity
    ::param modified: Flag to define the usage of the modified version 
    of the Schiller & Naumann correlation (including the inertial 
    range Re > 1000)
    
    :return: Schiller & Naumann (1993) drag coefficient
    :rtype: float
    
    :raises ValueError: if the Reynolds number value is zero or negative
    
    .. note::  References:
    * CROWE, Clayton T.; SCHWARZKOPF, John D.; SOMMERFELD, Martin; 
      TSUJI, Yutaka. Multiphase Flows with Droplets and Particles. 
      2. ed. Boca Raton, FL: CRC Press, 2012.
    * GIDASPOW, Dimitri. Multiphase Flow and Fluidization: 
      Continuum and Kinetic Theory  Descriptions. San Diego, 
    ...
    * ANSYS, Inc. CFX Solver Theory Guide. Canonsburg, PA, 2013.
    ...
    if reynolds <= 0.0:
        ...
        raise ValueError.with_traceback(trace_back)
    
    # CROWE et al. (2012), page 74, equation (4.55)
    drag_coefficient = 24.0 / reynolds * (1.0 + 0.15 * numpy.power(reynolds, 0.687))
    
    # Compute the modified version.
    # ANSYS (2013), section 5.5.2.1.1, equation (5-47)
    if modified:
        drag_coefficient = numpy.maximum(drag_coefficient, numpy.full_like(drag_coefficient, 0.44))
...
def Dallavalle1948(Re):
    ...
    C_D = numpy.power(0.63 + 4.8 / numpy.sqrt(Re), 2.0)
...
def HaiderLevenspiel1989(Re, phi = 1.0):
    ...
    # otherwise, calculate the coefficients based on the sphericity.
    A = numpy.where(phi == 1.0, 0.1806, numpy.exp(2.3288 - 6.4581 * phi + 2.4486 * numpy.power(phi, 2.0)))
    B = numpy.where(phi == 1.0, 0.6459, 0.0964 + 0.5565 * phi)
    C = numpy.where(phi == 1.0, 0.4251, numpy.exp(4.905 - 13.8944 * phi + 18.4222 * numpy.power(phi, 2.0) - 10.2599 * numpy.power(phi, 3.0)))
    D = numpy.where(phi == 1.0, 6880.95, numpy.exp(1.4681 + 12.2584 * phi - 20.7322 * numpy.power(phi, 2.0) + 15.8855 * numpy.power(phi, 3.0)))
        
    # drag coefficient calculation
    # HAIDER & LEVENSPIEL (1989), page 63, equation (4)
    C_D = 24.0 / Re * (1.0 + A * numpy.power(Re, B)) + C / (1.0 + D / Re)
...
def Ganser1993(Re, phi = 1.0, d_v = 1.0, d_n = 1.0, D = 1.0e+20):
    ...
    K_1 = 1.0 / (0.333333333333 * d_n / d_v + 0.666666666667 / numpy.sqrt(phi)) - 2.25 * d_v / D    
    
    # Newton form factor
    # GANSER (1993), page 151, table 7
    K_2 = numpy.power(10.0, numpy.power(1.8147 * (- numpy.log10(phi)), 0.5743))    
    
    # Ganser drag coefficient
    # GANSER (1993), page 150, equation (18)
    ReK1K2 = Re * K_1 * K_2
    C_D = K_2 * (24.0 / ReK1K2 * (1.0 + 0.1118 * numpy.power(ReK1K2, 0.6567)) + 0.4305 / (1.0 + 3305 / ReK1K2))
...
def WenYu1966(Re, alpha_f = 1.0):
    ...
    C_D = numpy.power(alpha_f, -1.65) * SchillerNaumann1933(Re * alpha_f)
...
def HuilinGidaspow2003(Re, alpha_f, phi = 1.0):
    ...
    psi = numpy.arctan(262.5 * (0.8 - alpha_f)) / numpy.pi + 0.5
...
def DiFelice1994(Re, alpha_f = 1.0):
    ...
    x = 1.5 - numpy.log10(alpha_f * Re)
    beta = 3.7 - 0.65 * numpy.exp(-(x * x) / 2.0);
    
    # DI FELICE (1994), pages 154 and 156, equations (6) and (21)
    C_D = C_D0 * numpy.power(alpha_f, 2.0-beta);
...
def HillKochLadd2001(Re, alpha_f = 1.0):
    ...
    w = numpy.exp(-10.0*(0.4 - alpha_s)/alpha_s)
    
    #calculating F0
    if alpha_s > 0.01 and alpha_s < 0.4:
        F0 = (1.0-w) * ((1.0+(3.0 * numpy.sqrt(alpha_s/2.0))
            + (135.0/64.0)*alpha_s*numpy.log(alpha_s) + 17.14*alpha_s ) / \
            (1.0 + 0.681 *alpha_s - 8.48 * alpha_s * alpha_s + 8.16 * alpha_s * alpha_s * alpha_s)) \
            + w* (10* alpha_s/numpy.power((1.0 - alpha_s), 3.0))
    else:
        F0 = (10* alpha_s/numpy.power((1.0 - alpha_s), 3.0))
        
    #calculating F1  
    if alpha_s > 0.01 and alpha_s < 0.1:
        F1 = numpy.sqrt(2.0/alpha_s)/40
    else:
        F1 = 0.11 + 0.00051 * numpy.exp(11.6* alpha_s)
    
    #calculating F2
    if alpha_s< 0.4:
        F2 = (1.0-w) * ((1.0+(3.0 * numpy.sqrt(alpha_s/2.0))
            + (135.0/64.0)*alpha_s*numpy.log(alpha_s) + 17.89*alpha_s )/(1.0 + 0.681 *alpha_s - 11.03 * alpha_s * alpha_s + 15.41 * alpha_s * alpha_s * alpha_s))\
            + w* (10* alpha_s/numpy.power((1.0 - alpha_s), 3.0))
    else:
        F2 = (10* alpha_s/numpy.power((1.0 - alpha_s), 3.0))
    
    #calculating F3
    if alpha_s<0.00953:
        F3 = 0.9351*alpha_s+0.03667
    else:
        F3 = 0.0673 + 0.212*alpha_s+ 0.0232/numpy.power((1.0-alpha_s), 5.0)
    ...
    elif alpha_s > 0.01 and Re_H <= (F3 + numpy.sqrt(F3*F3 - 4*F1 *(F0 - F2)))/(2*F1):
        F = F0 + F1*Re_H*Re_H
